...
# --- Streamlit runtime configurations ---
...
def load_excel_in_chunks(uploaded_file, dtype="str", chunksize=50000):
    """Load Excel in chunks to handle large files efficiently."""
    try:
        # Try reading normally first (small files)
        df = pd.read_excel(uploaded_file, dtype=dtype)
        return df
    except MemoryError:
        st.warning("Large file detected — using chunked read mode.")
        chunks = []
        reader = pd.read_excel(uploaded_file, dtype=dtype, chunksize=chunksize)
        for chunk in reader:
            chunks.append(chunk)
...
if daily_file and txn_file:
    ...
    daily_df = load_excel_in_chunks(daily_file)
    txn_df = load_excel_in_chunks(txn_file)

    # Validate structure
    required_daily_cols = {"Accession", "Test Code"}
    required_txn_cols = {"Inv.#", "Code", "TestName"}
    if not required_daily_cols.issubset(daily_df.columns) or not required_txn_cols.issubset(txn_df.columns):
        st.error("Column mismatch. Verify headers in both files.")
        st.stop()
    ...
    with tab1:
        ...
        st.write("**Daily Log Report (first 100 rows)**")
        st.dataframe(daily_df.head(100))
        st.write("**Transaction Report (first 100 rows)**")
        st.dataframe(txn_df.head(100))
        ...
        for idx, row in daily_df.iterrows():
            ...
            if accession in txn_map:
                if code in txn_map[accession]:
                    row_dict["Test Name"] = txn_testname_map.get((accession, code))
        ...
        st.subheader("Cleaned Daily Log Report (first 200 rows)")
        st.dataframe(cleaned_daily_df.head(200))
